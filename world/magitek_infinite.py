...
def create_infinite_magitek_floor_definition(floor):
    ...
    for variant in range(2):
        ...
        _catalog_mut.catalog_assign(template, 'MOB_TEMPLATES', MOB_TEMPLATES, (tid,))
...
_catalog_mut.catalog_setdefault_path('ROOMS', ROOMS, (), "magitek_zero_final", {}).setdefault("exits", {})["down"] = magitek_floor_id(1)
    ...
    if canonical not in ROOMS:
        ...
        if is_magitek_boss_floor(floor):
            ...
            if final_room in ROOMS:
                _catalog_mut.catalog_assign(f"Komora Bossa Magitek — piętro {floor}", 'ROOMS', ROOMS, (final_room, "name"))
                _catalog_mut.catalog_aug_path('ROOMS', ROOMS, (final_room, "desc"), 'Add', " To sala końcowego protokołu bossa tego piętra.")
...
def magitek_infinite_audit_v0382():
    ...
    for _rid in list(ROOMS):
        if _rid not in _rooms_before:
            _catalog_mut.catalog_pop_path('ROOMS', ROOMS, (), _rid, None)
    for _tid in list(MOB_TEMPLATES):
        if _tid not in _mobs_before:
            _catalog_mut.catalog_pop_path('MOB_TEMPLATES', MOB_TEMPLATES, (), _tid, None)
    return result
