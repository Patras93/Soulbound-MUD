...
# ----------------------------- v0.50.3 -----------------------------
V0503_DIFFICULTY_PRESSURE_VERSION = "0.50.3"

HELP_TOPICS.setdefault("trudnosc swiata", []).extend([
    "v0.50.3: świat jest trudniejszy bez przyspieszania końca gry: zwykły świat dostaje dodatkowo około +30% HP i +20% obrażeń ponad wcześniejszy finalny balans.",
    "Instancje są mocniejsze od świata: zwykłe lochy około +45% HP/+30% obrażeń, Krypty i Wieże około +55%/+38%, a Mityczne Krypty około +75%/+50%.",
    "Nagrody rosną znacznie słabiej niż trudność (zwykle +8% do +20%), więc większe ryzyko jest opłacalne, ale nie skraca długoterminowej progresji.",
])
HELP_TOPICS.setdefault("krypta", []).append(
    "v0.50.3: końcowa presja Krypty została zwiększona: zwykła Krypta +55% HP/+38% obrażeń, Mityczna +75% HP/+50% ponad poprzedni finalny balans; krzywa każdego piętra nadal obowiązuje."
)
HELP_TOPICS.setdefault("lochy", []).append(
    "v0.50.3: instancje i lochy mają dodatkową presję końcową; trudność rośnie mocniej niż EXP i waluta."
)

def difficulty_pressure_audit_v0503():
    errors = []
    required = {
        "world": (1.30, 1.20, 1.08),
        "instance": (1.45, 1.30, 1.12),
        "crypt": (1.55, 1.38, 1.15),
        "mythic_crypt": (1.75, 1.50, 1.20),
        "tower": (1.55, 1.38, 1.15),
        "mythic_tower": (1.70, 1.48, 1.20),
        "magitek": (1.65, 1.45, 1.18),
    }
    for kind, (min_hp, min_damage, max_reward) in required.items():
        row = V0503_DIFFICULTY_PRESSURE.get(kind)
        if not isinstance(row, dict):
            errors.append(f"missing pressure profile {kind}")
            continue
        hp = float(row.get("hp", 0) or 0)
        damage = float(row.get("damage", 0) or 0)
        reward = float(row.get("reward", 0) or 0)
        if hp < min_hp or damage < min_damage:
            errors.append(f"{kind}: pressure below target {row}")
        if reward > max_reward + 1e-9:
            errors.append(f"{kind}: reward pressure too high {reward}>{max_reward}")
        if hp <= reward or damage <= reward:
            errors.append(f"{kind}: rewards catch up with difficulty {row}")

    # Real final multiplier hierarchy at the same stage.
    probes = {
        "world": {"generator_level": 200},
        "instance": {"generator_level": 200, "profession_dungeon_floor": 200},
        "crypt": {"generator_level": 200, "crypt_floor": 200},
        "mythic_crypt": {"generator_level": 200, "mythic_crypt_floor": 200},
        "tower": {"generator_level": 200, "astral_floor": 200},
        "mythic_tower": {"generator_level": 200, "mythic_astral_floor": 200},
        "magitek": {"generator_level": 200, "machine": True},
    }
    final = {k: global_difficulty_multipliers_v0386(v) for k, v in probes.items()}
    if final["instance"]["hp"] <= final["world"]["hp"]:
        errors.append("instance HP not above world")
    if final["crypt"]["hp"] <= final["instance"]["hp"]:
        errors.append("crypt HP not above generic instance")
    if final["mythic_crypt"]["hp"] <= final["crypt"]["hp"]:
        errors.append("mythic crypt HP not above crypt")
    if final["mythic_tower"]["hp"] <= final["tower"]["hp"]:
        errors.append("mythic tower HP not above tower")

    return {
        "version": V0503_DIFFICULTY_PRESSURE_VERSION,
        "profiles": {k: dict(v) for k, v in V0503_DIFFICULTY_PRESSURE.items()},
        "sample_stage_200": final,
        "error_count": len(errors),
        "errors": errors,
    }
