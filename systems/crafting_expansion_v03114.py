...
for iid,(name,base,qty,level) in _REFINED.items():
    ITEMS[iid]={"name":name,"type":"craft_material","price":None,"craftbox_category":"refining",
                "desc":f"Materiał Refining 2.0. Powstaje z {ITEMS.get(base,{}).get('name',base)} x{qty}."}
    CRAFT_RECIPES[f"refine_{iid}"]={
...
ITEMS["socket_core_v03114"]={"name":"Rdzeń Gniazda","type":"craft_material","price":None,
    "craftbox_category":"runes","desc":"Zużywany przez Socket Crafting do dodania trwałego gniazda EQ."}
CRAFT_RECIPES["socket_core_v03114"]={
...
for set_id,(set_name,req_class,req_race,bonus) in TECH_SET_SPECS_V03114.items():
    ...
    for idx,slot in enumerate(TECH_SET_SLOTS_V03114):
        ...
        ITEMS[iid]=item; rows.append(iid)
        ...
        CRAFT_RECIPES[f"craft_{iid}"]={"name":item["name"],"aliases":(iid.replace('_',' '),),"stations":("forge",),
            "ingredients":ingredients,"output":iid,"quantity":1,"min_profession_level":220,
            "profession_xp":120,"tool_xp":80,"category":"tech_set","desc":f"Tech EQ Set — {set_name}."}
...
for iid,name,desc in (
    ...
    ITEMS[iid]={"name":name,"type":"craft_material","price":None,"craftbox_category":"technology","desc":desc}
...
for _rid,_r in TECH_CRAFT_RECIPES.items(): CRAFT_RECIPES[f"tech_{_rid}"]=_r
