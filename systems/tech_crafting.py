...
from systems.equipment_crafting import jewelry_socket_capacity
...
for _iid in TECH_COMPONENT_IDS:
    if _iid in ITEMS:
        _catalog_mut.catalog_assign("craft_material", 'ITEMS', ITEMS, (_iid, "type"))
        _catalog_mut.catalog_assign("technology", 'ITEMS', ITEMS, (_iid, "craftbox_category"))
        _catalog_mut.catalog_assign(True, 'ITEMS', ITEMS, (_iid, "machine_component"))
        _catalog_mut.catalog_assign(ITEMS[_iid].get("desc", "") + " Trafia do Szkatułki -> Technologia i służy do Tech Crafting/Salvage.", 'ITEMS', ITEMS, (_iid, "desc"))
...
_catalog_mut.catalog_update_path('ITEMS', ITEMS, (), _TECH_ITEMS)
...
for _rid,_recipe in TECH_CRAFT_RECIPES.items():
    _catalog_mut.catalog_assign(_recipe, 'CRAFT_RECIPES', CRAFT_RECIPES, (f"tech_{_rid}",))
