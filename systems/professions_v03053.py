...
# Rangi 1-400. Pierwsze 13 są opisowe, ostatnie 10 zgodne z progresją 201-400.
def _profession_rank_names(label, master):
    ...
    return base + tuple(f"{suffix} {label}" for suffix in _PROFESSION_400_RANK_SUFFIXES)
...
# Czterdzieści nazw Tierów na profesję; poziomy i progi pozostają wspólne 1-400.
def _tool_names(prefix):
    ...
    return tuple(f"{prefix} {x}" for x in stages+extra)
...
ITEMS.update({
 "tailor_kit":{"name":"Zestaw Krawiecki","type":"tool","tool_type":"tailoring","price":1200,"currency":"silver","desc":"Igły, nici i nożyce do Krawiectwa. Level 1-400, 40 Tierów, bez trwałości."},
 "tanning_knife":{"name":"Nóż Garbarski","type":"tool","tool_type":"leatherworking","price":1200,"currency":"silver","desc":"Narzędzie Garbarstwa. Level 1-400, 40 Tierów, bez trwałości."},
 "carpenter_tools":{"name":"Narzędzia Ciesielskie","type":"tool","tool_type":"carpentry","price":1200,"currency":"silver","desc":"Dłuta, piła precyzyjna i hebel do Stolarstwa. Level 1-400, 40 Tierów, bez trwałości."},
 "runic_focus":{"name":"Fokus Runiczny","type":"tool","tool_type":"enchanting","price":1200,"currency":"silver","desc":"Fokus do trwałego zaklinania wyposażenia. Level 1-400, 40 Tierów, bez trwałości."},
...
# Przykładowe pełne linie EQ 1-400. Każdy wynik ma inną tożsamość/statystyki.
def _armor(iid,name,slot,defense,affix,amount,level,profession):
    ITEMS[iid]={"name":name,"type":"armor","slot":slot,"defense":defense,"price":None,"rarity":"crafted","rarity_name":profession,"affix":affix,"affix_amount":amount,"generator_level":level,"desc":f"{profession} level {level}. Obrona +{defense}; {affix} +{amount}."}
...
for lvl,slot,name,affix in _tailor:
    ...
    if lvl>=100: mat['runestone_ore']=1
...
_carp_mats={1:'treated_plank',30:'oak_log',60:'ash_log',100:'runewood_log',160:'astralwood_log',220:'wood_400_220',300:'wood_400_300',400:'wood_400_400'}
...
COMMAND_ALIASES.update({'tailoring':'krawiectwo','leatherworking':'garbarstwo','carpentry':'stolarstwo','enchanting':'zaklinanie','sew':'szyj','tan':'garbuj','woodwork':'stolarka','enchant':'zaklinaj','zaklecia':'enchants'})
