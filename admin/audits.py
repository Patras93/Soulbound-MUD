...
def generator_whitelist_audit_v03019():
    ...
    errors = []
    if GENERATOR_CORE_VERSION != "0.33.10":
        errors.append(f"Generator Core version={GENERATOR_CORE_VERSION}, expected 0.33.10")
    if not audit.get("numeric_only"):
...
# ============================================================
# v0.33.10 - FULL GAME PRE-DEPLOY INTEGRITY GATE
...
def full_game_predeploy_audit_v0336():
    ...
    return {
        'version':'0.33.10','error_count':len(errors),'warning_count':len(warnings),
...
if _os_v0338.environ.get('SOULBOUND_FULL_AUDIT','').strip().lower() in ('1','true','yes','on'):
    ...
    if FULL_GAME_PREDEPLOY_AUDIT_V0336['error_count']:
        raise RuntimeError(
            'Full Game Pre-Deploy Audit v0.33.10 failed: '+
            '; '.join(map(str,FULL_GAME_PREDEPLOY_AUDIT_V0336['errors'][:100]))
        )
else:
    FULL_GAME_PREDEPLOY_AUDIT_V0336={
        'version':'0.33.10','skipped_at_runtime':True,'error_count':0,'warning_count':0,
        'errors':[],'warnings':[],
